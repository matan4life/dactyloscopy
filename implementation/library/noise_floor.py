...
def run(img, root):
    p = "%s/%s.png" % (WORK, root)
    img.save(p)
    subprocess.run(["mindtct", p, "%s/%s" % (WORK, root)], check=True,
                   capture_output=True)
    return [tuple(float(v) for v in l.split())
            for l in open("%s/%s.xyt" % (WORK, root))]


def mask_of(root, h):
    q = np.array([[int(v) for v in l.split()]
                  for l in open("%s/%s.qm" % (WORK, root))])
...
def measure(src_dir, tag):
    ...
    for k, name in enumerate(names):
        ...
        same = True
        for e in EXTS:
            ok = filecmp.cmp("%s/s.%s" % (WORK, e), "%s/s2.%s" % (WORK, e),
                             shallow=False)
            if not ok:
                det["files"][e] += 1
                same = False
