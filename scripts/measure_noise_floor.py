...
the aggregate as JSON. Run inside the container, with the corpus mounted and
LABDATA set to where it resolves there:
    ...
    docker run --rm -v "<repo>:/work" -v "$LABDATA/raw:/data/raw:ro" \
      ...
      dactyloscopy:dev python3 scripts/measure_noise_floor.py fvc2002/DB1_B
...
decision 3) and goes under $LABDATA/derived/inv016/<subset>/ unless an
output path is given.

Usage: measure_noise_floor.py <subset id> [<output json>]
"""
import argparse
import json
import os
...
def main(argv=None):
    ...
    parser.add_argument("out", nargs="?", default=None,
                        help="where the aggregate is written, as JSON; by "
                             "default $LABDATA/derived/inv016/<subset>/"
                             "noise_floor.json")
    ...
    if out is None:
        ...
        if not labdata:
            raise SystemExit("FAIL: LABDATA is required when no output path "
                             "is given")
        out = os.path.join(labdata, "derived", "inv016",
                           args.subset.replace("/", "_"), "noise_floor.json")
