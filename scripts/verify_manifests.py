...
The tree it verifies is the one this file sits in, so a copy of the tree is
verified by running the copy's own scripts/verify_manifests.py, which is how
docs/manifests.md exercises the failure path.

Usage: verify_manifests.py [corpus root]
"""
import argparse
import glob
import os
...
def main(argv=None):
    ...
    # A manifest that declares a distribution root finds its own data
    # through $LABDATA; this argument is an override for the case where it
    # does not.
    parser.add_argument("corpus", nargs="?", default=None,
                        help="the corpus root, when it is not where LABDATA "
                             "puts it")
    ...
    if corpus is None and not os.environ.get("LABDATA"):
        print("LABDATA is not set, so a manifest that names its data through "
              "it")
        print("finds nothing and its corpus checks are reported as not run.")
        print()

    manifests = sorted(glob.glob(os.path.join(REPO, "manifests",
                                              "MAN-*.json")))
    if not manifests:
        raise SystemExit("FAIL: manifests/MAN-*.json matched nothing")

    grand = {"total": 0, "checked": 0, "failed": 0, "refused": 0,
             "unchecked": 0}
    failures = []
    ...
    for path in manifests:
        ...
        try:
            ...
            print()
            failures.append("%s  <the whole file>  not readable as JSON"
                            % name)
            continue
    ...
    print("  %d checked, %d of them failing"
          % (grand["checked"], grand["failed"]))
    ...
    if grand["total"]:
        ...
        print("  no field carried a value and a status, so nothing was "
              "checked")
