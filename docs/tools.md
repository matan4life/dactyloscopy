...
the third tool was captured on 2026-09-07, and the two blocks dated
2026-09-22 were re-captured then. Each block names the command above
...
Re-captured on 2026-09-22. The 22 tests in the first file are this page's; the
10 in `tests/test_iso_xyt.py` hold the ISO-to-`.xyt` conversion `INV-018`
runs to the facts it implements; the 33 in `tests/test_manifest_verify.py`
are `docs/manifests.md`'s; the 13 in `tests/test_matching.py` hold the two
metrics `manifests/MAN-metrics.v1.json` registers to their definition texts;
the 4 in `tests/test_pool.py` hold the shared process pool to task order.
The suite needs no data: the reader's image is generated by the test from
constants the test states, and the metric tests state every score vector
they use. What it establishes about the reader, and what it cannot, is
written at the top of `tests/test_iso_extract.py`.
...
## Without the fixture directory
    ...
    make: *** [Makefile:89: check-tools] Error 1

Re-captured on 2026-09-22. The last two lines of the message date from
2026-09-06 and are older than the decision they say is missing: on 2026-09-07
`quality/REF-009` decision 1 kept the distributor's own layout under the raw
location, and `manifests/MAN-fvc2002.v1.json` records `distribution.root` as
`$LABDATA/raw/fvc2002` and the subset `fvc2002/DB1_B` at `Dbs/Db1_b` under
it. The target still takes the directory from `FVC_DB1_B` and still prints
the older sentence.
