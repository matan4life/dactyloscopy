...
33 in `tests/test_manifest_verify.py` are `docs/manifests.md`'s; the 13 in
`tests/test_matching.py` hold the two metrics `manifests/MAN-metrics.v1.json`
registers to their definition texts. The suite needs no data: the reader's
image is generated by the test from constants the test states, and the metric
tests state every score vector they use. What it establishes about the reader,
